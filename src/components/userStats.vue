      ...
      <template v-else-if="fullData">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
          <div class="bg-theme-primary p-5 rounded-2xl border border-thistle shadow-sm">
            <p class="text-[10px] font-black uppercase opacity-50 mb-6 text-center tracking-widest">andamento scambi (mensile)</p>
            <div class="h-[200px]">
          ...
          <div class="bg-theme-primary p-5 rounded-2xl border border-thistle shadow-sm">
            <p class="text-[10px] font-black uppercase opacity-50 mb-6 text-center tracking-widest">i tuoi tag</p>
            <div class="h-[200px]">
          ...
          <div class="md:col-span-2 bg-theme-primary p-5 rounded-2xl border border-thistle shadow-sm">
            <p class="text-[10px] font-black uppercase opacity-50 mb-6 text-center tracking-widest">i tuoi 5 titoli più richiesti</p>
            <div class="h-[200px]">
...
const simpleCards = computed(() => [
  { label: 'i tuoi libri', value: counters.myBooksCount, color: 'text-theme-main' },
...
const detailStats = computed(() => [
  { label: 'compagno di letture', value: fullData.value?.topPartner || 'nessuno' },
...
async function fetchCounters() {
  ...
  } catch (e) { console.error("Errore counters utente:", e); }
}
...
async function toggleOpen() {
  ...
  if (isOpen.value && !fullData.value) {
    ...
    } finally { isLoadingFull.value = false; }
  }
}
...
function renderCharts() {
  ...
  if (canvasPareto.value) {
    chartInstances.push(new Chart(canvasPareto.value, {
      type: 'bar',
      data: { 
        labels: fullData.value.paretoBooks.labels.map(t => t.length > 20 ? t.substring(0, 20) + '...' : t), 
        datasets: [{ data: fullData.value.paretoBooks.data, backgroundColor: '#629677', borderRadius: 4 }] 
      },
      options: {
        ...commonOptions,
        scales: { y: { ticks: { stepSize: 1, precision: 0 }, beginAtZero: true } }

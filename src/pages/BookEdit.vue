    ...
    <main class="max-w-4xl mx-auto p-6 space-y-10 relative">
        <div class="flex justify-between items-center mb-8">
            ...
            <div class="flex gap-4">
                <button @click="goBack" class="px-4 py-2 text-sm font-bold text-theme-main hover:text-zomp transition">
                    Annulla
                </button>
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-10 animate-fade-in">
            <section class="space-y-6 flex flex-col items-center">
                <div class="relative w-full aspect-[2/3] shadow-2xl rounded-2xl overflow-hidden bg-theme-primary border border-[var(--border-color)] group">
                ...
                <div class="w-full space-y-2">
                    <label class="block text-[10px] font-black text-theme-main uppercase tracking-widest ml-1 opacity-70">Stato prestito</label>
                    <select v-model="form.status" class="w-full p-3 rounded-xl border border-[var(--border-color)] bg-theme-primary text-theme-main focus:ring-2 focus:ring-zomp outline-none text-sm transition-colors">
            ...
            <section class="md:col-span-2 space-y-8">
                <div class="space-y-1">
                    <h2 class="text-3xl font-display text-theme-main">{{ form.title }}</h2>
                    <p class="text-sm text-theme-main opacity-60">Gestisci i dettagli della tua copia fisica</p>
                </div>

                <div class="space-y-6 text-theme-main">
                    <div>
                        <label class="block text-[10px] font-black uppercase tracking-widest mb-2 opacity-70">Condizioni</label>
                        <select v-model="form.condition" class="w-full p-4 rounded-xl border border-[var(--border-color)] bg-theme-primary text-theme-main focus:ring-2 focus:ring-zomp outline-none transition-colors">
                    ...
                    <div>
                        <label class="block text-[10px] font-black uppercase tracking-widest mb-2 opacity-70">Tags</label>
                        <div class="flex flex-wrap gap-2 p-3 border border-[var(--border-color)] rounded-xl bg-theme-primary min-h-[50px]">
                            ...
                            <input v-model="newTag" @keydown.enter.prevent="addTag" placeholder="Aggiungi tag..." class="flex-grow bg-transparent outline-none text-theme-main text-sm p-1">
                        </div>
                        <div class="mt-2 flex flex-wrap gap-2">
                            <button v-for="tag in commonTags" :key="tag" @click="addSuggestedTag(tag)" class="text-[9px] font-bold uppercase border border-[var(--border-color)] px-2 py-1 rounded hover:bg-ash-gray/20 transition">
                                + {{ tag }}
                            </button>
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block text-[10px] font-black uppercase tracking-widest mb-2 opacity-70">Note personali</label>
                        <textarea v-model="form.ownerNotes" rows="5" class="w-full p-4 rounded-xl border border-[var(--border-color)] bg-theme-primary text-theme-main focus:ring-2 focus:ring-zomp outline-none text-sm transition-colors" placeholder="Scrivi qui dettagli sulla tua copia..."></textarea>
                    </div>
                </div>
            </section>
        </div>

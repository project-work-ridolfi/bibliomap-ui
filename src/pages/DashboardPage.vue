  ...
  <div class="max-w-6xl mx-auto p-6 space-y-8 animate-fade-in text-theme-main font-sans">
    ...
    <div v-if="overdueLoans.length > 0" class="bg-red-100 dark:bg-red-900/20 border-2 border-red-500 p-6 rounded-2xl animate-pulse">
      <h3 class="text-red-700 dark:text-red-400 font-black flex items-center gap-2 uppercase tracking-tighter">
        <i class="fa-solid fa-triangle-exclamation"></i> azioni urgenti richieste
      </h3>
      <div class="mt-4 space-y-3">
        <div v-for="loan in overdueLoans" :key="loan.id" class="flex flex-col md:flex-row justify-between items-start md:items-center bg-white/50 dark:bg-black/20 p-4 rounded-xl border border-red-200 dark:border-red-800">
          <p class="text-sm font-bold text-red-800 dark:text-red-300 lowercase">devi restituire <span class="italic">{{ loan.title }}</span> (scaduto il {{ formatDate(loan.expectedReturnDate) }})</p>
          <button @click="openContactModal(loan)" class="mt-3 md:mt-0 bg-red-600 text-white px-4 py-2 rounded-lg text-[10px] font-bold uppercase hover:bg-red-700 transition">contatta proprietario</button>
        </div>
      </div>
    </div>

    <div class="flex flex-col md:flex-row justify-between items-end gap-4 border-b border-thistle pb-6">
      <div>
        <h1 class="text-3xl font-display text-theme-main lowercase">Centro <span class="text-zomp">Operativo</span></h1>
        <p class="opacity-60 text-sm">Gestisci i tuoi scambi e le tue letture in corso</p>
      </div>
      <div class="flex gap-3">
        <router-link to="/add-book" class="bg-zomp text-white px-5 py-2.5 rounded-xl font-bold text-[10px] uppercase shadow-md hover:opacity-90 transition flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
      ...
      <div class="lg:col-span-3 space-y-10">
        ...
        <section v-if="pendingRequests.length > 0" class="bg-tea-rose/10 border-l-4 border-tea-rose p-6 rounded-r-2xl space-y-4">
          <h3 class="font-display text-sm text-theme-main lowercase flex items-center gap-2">
            <i class="fa-solid fa-bell text-tea-rose"></i> nuove richieste di prestito
          </h3>
          <div class="space-y-3">
            <div v-for="req in pendingRequests" :key="req.id" class="flex justify-between items-center bg-theme-primary p-4 rounded-xl shadow-sm border border-thistle">
              <p class="text-sm">
                <span class="font-bold lowercase">{{ req.requesterUsername }}</span> vorrebbe leggere <span class="italic text-zomp">{{ req.title }}</span>
              </p>
              <button @click="openManageModal(req)" class="bg-zomp text-white px-4 py-2 rounded-lg text-[10px] font-bold uppercase">gestisci</button>
            </div>
          </div>
        </section>

        <section v-if="acceptedLoans.length > 0" class="bg-zomp/10 border-l-4 border-zomp p-6 rounded-r-2xl space-y-4">
          <h3 class="font-display text-sm text-theme-main lowercase flex items-center gap-2">
            <i class="fa-solid fa-handshake text-zomp"></i> scambi da confermare
          </h3>
          <div class="space-y-3">
            <div v-for="loan in acceptedLoans" :key="loan.id" class="flex justify-between items-center bg-theme-primary p-4 rounded-xl shadow-sm border border-thistle">
              <p class="text-sm">
                <template v-if="loan.ownerId === userData?.id">consegna <span class="font-bold">{{ loan.title }}</span> a <span class="text-zomp">{{ loan.requesterUsername }}</span></template>
                <template v-else>ricevi <span class="font-bold">{{ loan.title }}</span> da <span class="text-zomp">{{ loan.ownerUsername }}</span></template>
              </p>
              <button v-if="loan.ownerId === userData?.id" @click="confirmExchange(loan.id)" class="bg-zomp text-white px-4 py-2 rounded-lg text-[10px] font-bold uppercase">conferma consegna</button>
              <span v-else class="text-[10px] italic opacity-50 px-2 text-center leading-tight">in attesa del proprietario</span>
            </div>
          </div>
        </section>

        <section class="space-y-4">
          <div class="flex justify-between items-center border-b border-thistle pb-2">
            <h3 class="text-xl font-display text-theme-main lowercase">Stai leggendo:</h3>
            <div v-if="borrowedBooks.length > 3" class="flex gap-2">
              <button @click="prevBorrowed" :disabled="borrowedIndex === 0" class="p-1 disabled:opacity-20 hover:text-zomp transition"><i class="fa-solid fa-chevron-left"></i></button>
              <button @click="nextBorrowed" :disabled="borrowedIndex >= borrowedBooks.length - 3" class="p-1 disabled:opacity-20 hover:text-zomp transition"><i class="fa-solid fa-chevron-right"></i></button>
            </div>
          </div>
          <div v-if="borrowedBooks.length === 0" class="text-sm opacity-40 italic py-4">nessun prestito attivo in entrata</div>
          <div v-else class="relative overflow-hidden">
            <div class="flex transition-transform duration-500 ease-in-out gap-4" :style="{ transform: `translateX(-${borrowedIndex * (100 / 3 + 0.5)}%)` }">
              <div v-for="loan in borrowedBooks" :key="loan.id" class="w-full md:w-[calc(33.333%-11px)] shrink-0 bg-theme-primary p-5 rounded-xl shadow-sm border border-thistle flex flex-col justify-between min-h-[160px]">
                <div>
                  <h4 class="font-bold text-zomp text-lg leading-tight truncate">{{ loan.title }}</h4>
                  <p class="text-[10px] opacity-60 lowercase">Proprietario: {{ loan.ownerUsername }}</p>
                </div>
                <div class="mt-4 flex justify-between items-end pt-3 border-t border-thistle/50">
                  <div class="min-w-0">
                    <p class="text-[8px] uppercase font-black opacity-40">scadenza</p>
                    <p class="text-xs font-mono" :class="isOverdue(loan.expectedReturnDate) ? 'text-red-500 font-bold' : ''">{{ formatDate(loan.expectedReturnDate) }}</p>
                  </div>
                  <button @click="openContactModal(loan)" class="text-[9px] font-bold border border-thistle px-2 py-1.5 rounded-lg hover:bg-ash-gray/20">contatta</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <section class="space-y-4">
          <div class="flex justify-between items-center border-b border-thistle pb-2">
            <h3 class="text-xl font-display text-theme-main lowercase">Hai prestato:</h3>
            <div v-if="lentBooks.length > 3" class="flex gap-2">
              <button @click="prevLent" :disabled="lentIndex === 0" class="p-1 disabled:opacity-20 hover:text-zomp transition"><i class="fa-solid fa-chevron-left"></i></button>
              <button @click="nextLent" :disabled="lentIndex >= lentBooks.length - 3" class="p-1 disabled:opacity-20 hover:text-zomp transition"><i class="fa-solid fa-chevron-right"></i></button>
            </div>
          </div>
          <div v-if="lentBooks.length === 0" class="text-sm opacity-40 italic py-4">non hai libri attualmente in prestito ad altri</div>
          <div v-else class="relative overflow-hidden">
            <div class="flex transition-transform duration-500 ease-in-out gap-4" :style="{ transform: `translateX(-${lentIndex * (100 / 3 + 0.5)}%)` }">
              <div v-for="loan in lentBooks" :key="loan.id" class="w-full md:w-[calc(33.333%-11px)] shrink-0 bg-theme-primary p-5 rounded-xl shadow-sm border border-thistle flex flex-col justify-between min-h-[160px]">
                <div>
                  <h4 class="font-bold text-theme-main text-lg leading-tight truncate">{{ loan.title }}</h4>
                  <p class="text-[10px] opacity-60 lowercase">Lettore: {{ loan.requesterUsername }}</p>
                </div>
                <div class="mt-4 flex flex-col gap-2 pt-3 border-t border-thistle/50">
                  <button @click="openReturnModal(loan)" class="w-full bg-zomp text-white py-2 rounded-lg font-bold text-[9px] uppercase tracking-wider shadow-sm">segnala rientro</button>
                  <button @click="openExtendModal(loan)" class="w-full border border-thistle text-theme-main py-2 rounded-lg font-bold text-[9px] uppercase opacity-70">allunga scadenza</button>
                </div>
              </div>
            </div>
          </div>
      ...
      <aside class="lg:col-span-1 space-y-6">
        <section class="bg-theme-primary p-6 rounded-2xl shadow-md border border-thistle space-y-6">
          <h3 class="font-display text-xs uppercase text-zomp tracking-widest border-b border-thistle pb-2">Navigazione</h3>
        ...
        <section v-if="userData" class="p-5 bg-ash-gray/10 rounded-2xl border border-dashed border-thistle">
          <p class="text-[9px] font-black uppercase opacity-40 mb-2">Privacy Corrente</p>
          <p class="text-xs font-bold text-theme-main lowercase">Visibilità: <span class="text-zomp">{{ formatVisibility(userData.visibility) }}</span></p>
          <p class="text-[10px] opacity-60 mt-1 leading-tight">Puoi cambiare chi vede la tua posizione nelle impostazioni del profilo.</p>
        </section>
      </aside>
...
onMounted(async () => {
  await fetchUserMe();
  await Promise.all([fetchIncomingRequests(), fetchActiveLoans(), fetchAcceptedLoans()]);
});
...
async function confirmExchange(id) { await apiClient.post(`/loan/${id}/start`); await Promise.all([fetchAcceptedLoans(), fetchActiveLoans()]); }
async function confirmReturn() { await apiClient.post(`/loan/${selectedLoan.value.id}/return`, { condition: returnForm.condition }); isReturnModalOpen.value = false; fetchActiveLoans(); }
async function confirmExtension() { await apiClient.patch(`/loan/${selectedLoan.value.id}/extend`, { days: extendForm.days }); isExtendModalOpen.value = false; fetchActiveLoans(); }
...
async function handleModalSubmit() {
  ...
  if (isContactMode.value) { await apiClient.post(`/loan/${selectedRequest.value.id}/contact`, payload); }
    ...
    await apiClient.patch(`/loan/${selectedRequest.value.id}/status`, { ...payload, action: modalForm.action }); 
...
const formatVisibility = (v) => ({ all: 'Tutti', logged_in: 'Registrati', private: 'Privato' }[v] || 'Privato');
</script>
...
.nav-link i { opacity: 0.5; width: 16px; }
.nav-link:hover i { opacity: 1; }
.btn-outline { border: 2px solid var(--zomp); color: var(--zomp); font-weight: bold; border-radius: 12px; transition: all 0.2s; }
.btn-outline:hover { background: var(--zomp); color: white; }
</style>
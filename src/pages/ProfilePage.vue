<template>
  <main class="max-w-7xl mx-auto p-6 space-y-8 animate-fade-in text-theme-main font-sans">
    <button
      @click="router.back()"
      ...
      <aside class="md:col-span-1 space-y-6">
        ...
        <section class="bg-theme-primary p-5 rounded-xl shadow border border-thistle space-y-4">
          ...
          >
            ...
            <span class="tracking-widest">{{ isEditMode ? 'VEDI PROFILO' : 'GESTISCI PROFILO' }}</span>
          </button>
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div class="space-y-8">
              <div class="space-y-4">
                ...
                <button @click="handleUpdateUsername" :disabled="!isUsernameModified || !usernameAvailability || isCheckingUsername" class="btn-primary uppercase">
                ...
                <div class="pt-6 border-t border-thistle mt-6">
                  ...
                  <div v-else class="space-y-4">
                    ...
                    <div>
                      ...
                      <div v-if="passwordForm.new" class="mt-2 p-2 rounded-lg bg-red-50 border border-red-200">
                        <ul class="text-[9px] space-y-1 font-bold uppercase">
                          <li :class="passRequirements.minLength ? 'text-zomp' : 'text-red-500'">• almeno otto caratteri</li>
                    ...
                    <div class="flex gap-2">
                      ...
                      <button @click="cancelPasswordChange" class="flex-1 btn-outline uppercase">chiudi</button>
                    </div>
                  </div>
                </div>
              </div>
    ...
    <AppModal :is-open="legalModal.open" :title="legalModal.title" @close="legalModal.open = false">
      <div v-html="legalModal.content" class="prose dark:prose-invert max-w-none text-sm px-4 py-2 text-theme-main"></div>
    </AppModal>
  </main>
</template>
...
const visibilityIta = computed(() => {
  ...
  return { all: 'pubblico', 'logged-in': 'utenti registrati', private: 'privato' }[v] || 'privato';
});
...
async function fetchProfileData() {
  ...
  try {
    ...
    if (isMyProfile.value) {
      Object.assign(profileForm, { username: resProfile.userName, visibility: resProfile.visibility, blurRadius: resProfile.blurRadius });
    }
...
const isPasswordValid = computed(() => Object.values(passRequirements).every(v => v) && passwordForm.new === passwordForm.confirm);
...
async function handleUpdateUsername() {
  ...
  fetchProfileData();
  uiModal.value = { open: true, title: "Successo", message: "Username aggiornato" };
}

async function updatePassword() {
  try {
    await apiClient.put(`/users/${authStore.userId}/password`, { old: passwordForm.old, new: passwordForm.new });
    cancelPasswordChange();
    uiModal.value = { open: true, title: "Successo", message: "Password modificata" };
  } catch (e) {
    uiModal.value = { open: true, title: "Errore", message: "Password attuale errata" };
  }
}

async function handlePrivacyUpdate() {
  await apiClient.put(`/users/${authStore.userId}/privacy`, { visibility: profileForm.visibility, blurRadius: profileForm.blurRadius });
  await authStore.fetchCurrentUser();
  fetchProfileData();
  uiModal.value = { open: true, title: "Privacy", message: "Impostazioni salvate" };
}

async function handleExportRequest() {
  try {
    const res = await apiClient.get("/users/export");
    uiModal.value = { open: true, title: "Esportazione Dati", message: res.message || "Richiesta inviata con successo. Riceverai i dati via email." };
  } catch (e) {
    uiModal.value = { open: true, title: "Errore", message: "Impossibile completare la richiesta di export." };
  }
}
...
function cancelPasswordChange() { isChangingPassword.value = false; Object.assign(passwordForm, { old: "", new: "", confirm: "" }); }
function openLegal(t) { legalModal.value = { open: true, title: t === 'terms' ? 'Termini e Condizioni' : 'Privacy Policy', content: t === 'terms' ? TERMS_AND_CONDITIONS : PRIVACY_POLICY }; }
function downloadPDF(t) { downloadAsPDF(t === 'terms' ? TERMS_AND_CONDITIONS : PRIVACY_POLICY, `bibliomap_${t}`); }
...
async function initEditMap() {
  ...
  mapInstanceEdit = new maplibregl.Map({
    ...
    style: `https://api.maptiler.com/maps/019a4997-dd19-75e4-bf35-d09baea3fb61/style.json?key=${import.meta.env.VITE_MAPTILER_KEY}`,
    center: [profile.value.coords.longitude, profile.value.coords.latitude],
    zoom: 12,
    interactive: false
  });
  new maplibregl.Marker({ color: "#629677" }).setLngLat([profile.value.coords.longitude, profile.value.coords.latitude]).addTo(mapInstanceEdit);
}
...
async function calculateAffinity() {
  try {
    ...
    affinityMessage.value = affinityScore.value > 50 ? "Anime gemelle letterarie!" : "Scoprite nuovi generi insieme.";
  } catch (e) { affinityScore.value = 0; }
}

watch(() => profileForm.username, (newVal) => {
  if (!isUsernameModified.value) { usernameAvailability.value = true; return; }
...
.input-field { width: 100%; padding: 0.75rem; border: 2px solid var(--ash-gray); border-radius: 0.75rem; background: transparent; outline: none; transition: border-color 0.2s; }
.input-field:focus { border-color: var(--zomp); }
.btn-primary { width: 100%; background: var(--zomp); color: white; padding: 0.8rem; border-radius: 0.75rem; font-weight: bold; font-size: 11px; text-transform: uppercase; transition: all 0.2s; }
.btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }
.btn-outline { width: 100%; border: 2px solid var(--zomp); color: var(--zomp); padding: 0.75rem; border-radius: 0.75rem; font-weight: bold; font-size: 11px; text-transform: uppercase; background: transparent; transition: all 0.2s; }
.btn-outline:hover { background: var(--zomp); color: white; }
.btn-red { background: #ef4444; color: white; border-radius: 0.75rem; font-weight: bold; font-size: 11px; }
.animate-fade-in { animation: fadeIn 0.4s ease-out forwards; }
@keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
input[type="range"] { accent-color: var(--zomp); }
</style>